		...
		for (int i = fd_max; i >= 0; --i)
		{
			if (FD_ISSET(i, &tmp_fds) != 0)
			{
				...
				if (sock_tcp - i == 0)
				{
					...
					int n = recv(newsockfd, new_client_id, 11, 0);
					...
					if (!(all_clients.end() ==
						  all_clients.find(new_client_id)))
					{
						tcpC client;
						client = all_clients.at(new_client_id);
						...
						{
							for (auto to_send = store_and_forward.find(client.id)->second.begin();
								 to_send != store_and_forward.find(client.id)->second.end(); ++to_send)
							{
				...
				if (i == sock_udp)
				{
					...
					int szfrmst;
					szfrmst = sizeof(Rstation);

					int n;
					n = recvfrom(sock_udp, buffer, 1550 + 1, 0,
					...
					memcpy(&message.content, buffer + 50 + 1, 1500);
					...
					for (auto s = topics_subs.find(message.topic)->second.begin();
						 s != topics_subs.find(message.topic)->second.end(); s++)
					{
						tcpC client;
						client = s->client;
						...
						if ((s->sf == '0') && (active_ids_sockets.find(client.id) ==
											   active_ids_sockets.end()))
						{
							continue;
						}
				...
				if (req.req_type == 0)
				{
					for (auto s = topics_subs.find(req.topic)->second.begin();
						 s != topics_subs.find(req.topic)->second.end(); ++s)
					{
